...
#define QUERYA_GTIM_TIMX_CAP_CHY_GPIO_AF           GPIO_AF1_TIM2                                 /* AF功能选择 */
...
#define QUERYA_GTIM_TIMX_CAP_CHY_CCRX              TIM2->CCR1                                    /* 通道Y的输出比较寄存器 */
#define QUERYA_GTIM_TIMX_CAP_CHY_CLK_ENABLE()      do{ __HAL_RCC_TIM2_CLK_ENABLE(); }while(0)    /* TIM2 时钟使能 */

#define QUERYB_GTIM_TIMX_CAP_CHY_GPIO_PORT         GPIOB
#define QUERYB_GTIM_TIMX_CAP_CHY_GPIO_PIN          GPIO_PIN_3
#define QUERYB_GTIM_TIMX_CAP_CHY_GPIO_CLK_ENABLE() do{ __HAL_RCC_GPIOB_CLK_ENABLE(); }while(0)   /* PA口时钟使能 */
...
//#define GTIM_TIMX_CAP_IRQHandler                   TIM2_IRQHandler
#define QUERYB_GTIM_TIMX_CAP_CHY                   TIM_CHANNEL_2                                 /* 通道Y,  1<= Y <=4 */
...
/******************************************************************************************/
void queryA_gtim_timx_cap_chy_init(uint32_t arr, uint16_t psc);                   /* 通用定时器 queryA输入捕获7ms query信号初始化函数 */
void queryB_gtim_timx_cap_chy_init(uint32_t arr, uint16_t psc);                   /* 通用定时器 queryB输入捕获7ms query信号初始化函数 */
void queryA_gtim_timx_cap_tm_init(uint32_t arr, uint16_t psc);                    /* 通用定时器 queryA输入捕获TM返回的35ms脉冲初始化函数 */
