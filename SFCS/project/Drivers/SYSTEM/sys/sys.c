 ...
 */
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq, uint32_t pllr)
{
	...
	//ret = HAL_RCC_ClockConfig(&rcc_clk_init, FLASH_LATENCY_1);   /* 同时设置FLASH延时周期为4WS，也就是5个CPU周期 */
    ret = HAL_RCC_ClockConfig(&rcc_clk_init, FLASH_LATENCY_3);   /* 同时设置FLASH延时周期为4WS，也就是5个CPU周期 */
    if(ret != HAL_OK)
    {
        return 1;                                                /* 时钟初始化失败 */
    }
