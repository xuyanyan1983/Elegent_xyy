...
extern TIM_HandleTypeDef g_tim1_queryA_cap_handle; /* 定时器x句柄 */
...
extern uint8_t g_tim1_queryA_cap_sta;       /* 输入捕获状态 */
extern uint16_t g_tim1_queryA_cap_val;      /* 输入捕获值 */
...
{
	if(g_tim1_queryA_cap_sta & 0x80)                                     /* 成功捕获到了一次高电平 */
	{
		queryAtemp = g_tim1_queryA_cap_sta & 0x3F;
		queryAtemp *= 0xFFFF;                                            /* 溢出时间总和 */
		queryAtemp += g_tim1_queryA_cap_val;                             /* 得到总的高电平时间 */
		/*捕获到5ms的query信号*/
		if((queryAtemp >= (5*1000-500))&&(queryAtemp <= (5*1000+500)))
		{
			...
			HAL_TIM_IC_Stop_IT(&g_tim1_queryA_cap_handle, QUERYA_GTIM_TIMX_CAP_CHY);
		...
		g_tim1_queryA_cap_sta = 0;                                       /* 开启下一次捕获 */
	}
...
void tmCapQueryDisposeFunc()
{
	if(g_tim1_queryA_cap_sta & 0x80)                                     /* 成功捕获到了一次高电平 */
	{
		queryATmtemp = g_tim1_queryA_cap_sta & 0x3F;
		queryATmtemp *= 0xFFFF;                                            /* 溢出时间总和 */
		queryATmtemp += g_tim1_queryA_cap_val;                             /* 得到总的高电平时间 */
		/*捕获到5ms的query信号*/
...
void queryCheckDisposeFunc(uint8_t normalPulse,uint8_t abnormalPulse)
{
	...
	if(query_on_switch == 0x01)
	{
		...
		HAL_TIM_IC_Start_IT(&g_tim1_queryA_cap_handle, QUERYA_GTIM_TIMX_CAP_CHY);
		/*捕获到TM返回的35ms脉冲*/
...
uint8_t tmNodeDisposeFunc(NodeDataStruct nodeData)
{
	...
	{
		can_send_msg_data(0x02800000|(NodeId<<16), canbuf, 2);     /*当电子开关闭合时，上报数据 2字节*/
	}		
    return 0;
}
