...
void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc)
{
	...
	GPIO_InitStruct.Pin = ADCx_CHANNEL3_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(ADCx_CHANNEL3_GPIO_PORT, &GPIO_InitStruct);
}
...
void HAL_ADC_MspDeInit(ADC_HandleTypeDef *hadc)
{
	...
	HAL_GPIO_DeInit(ADCx_CHANNEL3_GPIO_PORT, ADCx_CHANNEL3_PIN);
}
...
uint16_t adc_sigChann_get_value(ADC_HandleTypeDef AdcHandle,ADC_ChannelConfTypeDef sConfig,uint8_t ch)
{
  ...
  {
	sConfig.Channel      = ADCx_CHANNEL3;                /* Sampled channel number */
  }
