...
void rcmdi_init(void)
{
	...
	RCMDI2_GPIO_CLK_ENABLE();
	RCMDI3_GPIO_CLK_ENABLE();
	RCMDI4_GPIO_CLK_ENABLE();
	RCMDI5_GPIO_CLK_ENABLE();
	RCMDI6_GPIO_CLK_ENABLE();
	RCMDI7_GPIO_CLK_ENABLE();
	RCMDI8_GPIO_CLK_ENABLE();
	...
	HAL_NVIC_SetPriority(EXTI4_IRQn, 0, 4);
...
void AudioCheck_init(void)
{
    ...
    AUDIO_OUT_GPIO_CLK_ENABLE();                             /* AUDIO_OUT时钟使能 */
	IN_SD_AU_GPIO_CLK_ENABLE();
	
    gpio_init_struct.Pin = AUDIO_OUT_GPIO_PIN;               /* AUDIO_OUT引脚 */
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;            /* 推挽输出 */
    gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
    HAL_GPIO_Init(AUDIO_OUT_GPIO_PORT, &gpio_init_struct);   /* 初始化IN_SD_AU引脚 */
	
	gpio_init_struct.Pin = IN_SD_AU_GPIO_PIN;                      /* IN_SD_AU按钮引脚 */
    gpio_init_struct.Mode = GPIO_MODE_INPUT;                      /* 输入 */
    gpio_init_struct.Pull = GPIO_NOPULL;                          /* 上拉 */
    HAL_GPIO_Init(IN_SD_AU_GPIO_PORT, &gpio_init_struct);          /* IN_SD_AU按钮引脚模式设置,上拉输入 */
}
...
void lock_init(void)
{
	...
	AC_LOCK_GPIO_CLK_ENABLE();
	IN_BTN2_GPIO_CLK_ENABLE();
	...
	gpio_init_struct.Pin = AC_LOCK_GPIO_PIN;                /*AC lock引脚 */
    gpio_init_struct.Mode = GPIO_MODE_INPUT;                /* 推挽输出 */
    gpio_init_struct.Pull = GPIO_NOPULL;                    /* 不用上下拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
    HAL_GPIO_Init(AC_LOCK_GPIO_PORT, &gpio_init_struct);  /* 初始化lock stop引脚 */
	
	/*lock按钮*/
	gpio_init_struct.Pin = IN_BTN2_GPIO_PIN;               /* IN_BTN2引脚 */
    gpio_init_struct.Mode = GPIO_MODE_INPUT;               /* 推挽输出 */
    gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
    HAL_GPIO_Init(IN_BTN2_GPIO_PORT, &gpio_init_struct);   /* 初始化IN_SD_AU引脚 */
}
